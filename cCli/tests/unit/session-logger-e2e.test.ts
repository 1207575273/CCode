...
describe('E2E: 完整对话链路', () => {
  ...
  it('纯文本对话 — session_start → user → llm_start/end → assistant → session_end', () => {
    ...
    logger.consume({ type: 'llm_done', inputTokens: 100, outputTokens: 50, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
  ...
  it('工具调用链路 — llm → tool_start/end → llm → assistant', () => {
    ...
    // 第一轮 LLM：决定调用工具
    logger.consume({ type: 'llm_start', provider: 'anthropic', model: 'claude', messageCount: 1 })
    logger.consume({ type: 'llm_done', inputTokens: 200, outputTokens: 30, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
    ...
    logger.consume({ type: 'llm_done', inputTokens: 500, outputTokens: 100, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
  ...
  it('MCP 连接事件 — 在 LLM 调用前触发', () => {
    ...
    // LLM 调用
    logger.consume({ type: 'llm_start', provider: 'anthropic', model: 'claude', messageCount: 1 })
    logger.consume({ type: 'llm_done', inputTokens: 50, outputTokens: 20, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
    logger.consume({ type: 'done' })
  ...
  it('parentUuid 链完整性 — 所有事件形成单链', () => {
    ...
    logger.logUserMessage('test')
    logger.consume({ type: 'llm_start', provider: 'anthropic', model: 'claude', messageCount: 1 })
    logger.consume({ type: 'llm_done', inputTokens: 10, outputTokens: 5, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
  ...
  it('start/end 配对完整性 — 每种 start 都有对应的 end', () => {
    ...
    logger.consume({ type: 'llm_done', inputTokens: 20, outputTokens: 10, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
    logger.consume({ type: 'done' })
