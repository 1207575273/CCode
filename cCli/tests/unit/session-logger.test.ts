...
describe('SessionLogger.consume', () => {
  ...
  it('should_write_llm_call_end_event_with_tokens', () => {
    logger.ensureSession('anthropic', 'claude')
    const added = consumeAndRead({ type: 'llm_done', inputTokens: 100, outputTokens: 50, cacheReadTokens: 0, cacheWriteTokens: 0, stopReason: 'end_turn', ttftMs: 0, e2eMs: 0, tps: 0 })
    expect(added).toHaveLength(1)
