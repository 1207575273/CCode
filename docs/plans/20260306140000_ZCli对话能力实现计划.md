...
**Architecture:** `useChat` hook 管理流式状态，调用 `LLMProvider` 接口，底层用 LangChain `ChatAnthropic` / `ChatOpenAI`；配置读自 `~/.zcli/config.json`；ChatView 接收 `streamingMessage` prop 渲染打字机效果。
...
git add zCli/tsconfig.json zCli/vitest.config.ts
git commit -m "chore: 添加 @config 路径别名"
```

---

## Task 2: ConfigManager — 读写 ~/.zcli/config.json
...
```bash
cd D:/a_dev_work/claude_cli_z01
git add zCli/src/config/config-manager.ts zCli/tests/unit/config-manager.test.ts
git commit -m "feat: ConfigManager — 读写 ~/.zcli/config.json"
```
...
```bash
cd D:/a_dev_work/claude_cli_z01
git add zCli/src/providers/message-converter.ts zCli/tests/unit/message-converter.test.ts
...
```bash
cd D:/a_dev_work/claude_cli_z01
git add zCli/src/providers/anthropic.ts zCli/tests/unit/anthropic-provider.test.ts
...
```bash
cd D:/a_dev_work/claude_cli_z01
git add zCli/src/providers/openai-compat.ts zCli/tests/unit/openai-compat-provider.test.ts
...
```bash
cd D:/a_dev_work/claude_cli_z01
git add zCli/src/providers/registry.ts zCli/tests/unit/provider-registry.test.ts
...
```bash
cd D:/a_dev_work/claude_cli_z01
git add zCli/src/ui/ChatView.tsx zCli/src/ui/InputBar.tsx
...
在 `~/.zcli/config.json` 填入有效的 apiKey，然后：
