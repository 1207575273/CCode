...
| `bin/zcli.ts` | 修改 | 退出时调用 tokenMeter.flush() |
...
**Files:**
- Modify: `zCli/src/core/agent-loop.ts:32-45`（AgentEvent 类型）
- Modify: `zCli/src/core/agent-loop.ts:126-159`（#callLLM 方法）
- Modify: `zCli/src/persistence/session-types.ts:51-83`（SessionEvent 字段）
- Modify: `zCli/src/observability/session-logger.ts:20-27,70-78,190-202`（TurnStats + consume + finalize）
...
**Files:**
- Create: `zCli/src/persistence/db.ts`
- Modify: `zCli/src/persistence/index.ts`（导出 getDb）
- Test: `zCli/tests/unit/db.test.ts`
...
**Files:**
- Create: `zCli/src/observability/token-meter.ts`
- Modify: `zCli/src/observability/index.ts`（导出 TokenMeter）
- Test: `zCli/tests/unit/token-meter.test.ts`
...
**Files:**
- Modify: `zCli/src/ui/useChat.ts:32-34,84,208-209,213-215`
- Modify: `zCli/bin/zcli.ts:50-56`
...
- [ ] **Step 4: 在 zcli.ts 退出时关闭数据库**

在 `zcli.ts:5` 添加 import：
...
在 `zcli.ts:50-56` 的 exitGracefully 中，`sessionLogger.finalize()` 之后添加：
...
```bash
git add src/ui/useChat.ts bin/zcli.ts
...
**Files:**
- Create: `zCli/src/commands/usage.ts`
- Modify: `zCli/src/commands/types.ts`（CommandAction 新增 show_usage）
- Modify: `zCli/src/ui/App.tsx`（注册 /usage、处理 show_usage action、StatusBar 区域）
...
2. 输入 `/usage` 是否展示三级汇总
3. 检查 `~/.zcli/data/zcli.db` 是否创建，pricing_rules 是否有默认数据
4. 查看 usage_logs 是否有记录：`sqlite3 ~/.zcli/data/zcli.db "SELECT * FROM usage_logs"`
